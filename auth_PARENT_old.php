...
if ($stmt = $db->prepare('SELECT id, fullname, pw, klasse, barn, tlf_nr, adress FROM users WHERE username = ?')) {
	// Bind parameters (s = string, i = int, b = blob, etc), in our case the username is a string so we use "s"
	$stmt->bind_param('s', $_POST['username']);
	$stmt->execute();
	// Store the result so we can check if the account exists in t he database.
	$stmt->store_result();
}

if ($stmt->num_rows > 0) {
	$stmt->bind_result($id, $fullname, $password, $class, $child, $phone_nr, $adress);
	$stmt->fetch();
